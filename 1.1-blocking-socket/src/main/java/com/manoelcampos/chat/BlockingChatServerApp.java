...
public class BlockingChatServerApp {
    ...
    private void sendMsgToOne(final ClientSocket sender, final String msg) {
        if(msg == null || msg.isEmpty()) {
            System.err.println("A mensagem não pode ser vazia");
            return;
        }
        
        final Iterator<ClientSocket> iterator = clientSocketList.iterator();
        int i = msg.indexOf(" ");
        final String login = msg.substring(1, i);        
        final String mensagem = msg.substring(i, msg.length());
        ...
        while (iterator.hasNext()) {
            ...
            /*Verifica se o elemento atual da lista (cliente) não é o cliente que enviou a mensagem.
            Se não for, encaminha a mensagem pra tal cliente.*/
            System.out.printf("login atual: '%s' login enviado: '%s'\n", client.getLogin(), login);
            if (!client.equals(sender) && client.getLogin().equals(login)) {
                if(client.sendMsg(msg)){
                    System.out.println("Mensagem encaminhada pro cliente " + login);    
                    return;                                            
                }
            }
        }
